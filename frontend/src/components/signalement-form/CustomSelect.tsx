...
export const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  return (
    ...
    >
      <Select.Trigger
        className={`w-full px-3.5 py-2.5 rounded-xl border text-sm flex items-center justify-between text-left transition-all outline-none bg-white dark:bg-emc-elevated/90 text-slate-900 dark:text-emc-primary ${
          error
      ...
      >
        <Select.Value className="text-sm font-medium pr-2 text-left leading-snug">
          {selectedOption ? (
        ...
        <Select.Indicator>
          <ChevronDown className="w-4 h-4 text-slate-400 dark:text-emc-muted-fg flex-shrink-0" />
        </Select.Indicator>
      </Select.Trigger>
